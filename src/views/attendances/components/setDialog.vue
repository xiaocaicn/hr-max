<template>
  <div class="add-form">
    <el-dialog title="设置" :visible="dialogFormVisible" @close="handleClose">
...
import { attendanceSave, getAttendance, updataAttendanceLeave, getAttendanceLeave, getDeductions, deductionsSave, overtimeSave, getOvertime } from '@/api/attendances'
export default {
  name: 'Add',
  props: {
    dialogFormVisible: {
  ...
  methods: {
    ...
    async handleChange(value) {
      console.log(value)
      const { morningStartTime, morningEndTime, afternoonStartTime, afternoonEndTime } = await getAttendance({ departmentId: value })
      this.formBase.morningStartTime = morningStartTime
      this.formBase.morningEndTime = morningEndTime
      this.formBase.afternoonStartTime = afternoonStartTime
      this.formBase.afternoonEndTime = afternoonEndTime
    },
    ...
    handleLeave() {
      this.$refs.leaveForm.validate(async valid => {
        if (valid) {
          ...
          await updataAttendanceLeave(this.tylelist)
          this.$message.success('保存成功')
          this.handleClose()
        }
      })
    },
    async getAttendanceLeave() {
      console.log(this.leaveBase.departmentId)
      const res = await getAttendanceLeave({ departmentId: this.leaveBase.departmentId })
      console.log(res)
      // this.leaveBase = res
      res.forEach(item => {
        if (item.isEnable === 0) {
    ...
    handleDeductions() {
      this.$refs.deductionsForm.validate(async valid => {
        if (valid) {
          const deductionList = this.stateData.departmentType
          deductionList.forEach(item => {
            item.dedAmonutLowerLimit = 30
            item.dedAmonutUpperLimit = 0
            if (item.isEnable) {
              item.isEnable = 0
            } else {
              item.isEnable = 1
          ...
          console.log(deductionList[0])
          await deductionsSave(deductionList[0])
          this.handleClose()
        }
      })
    },
